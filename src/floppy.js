    ...
    if (this.response_cursor < this.response_length) {
        ...
        if (this.response_cursor === this.response_length) {
            const lower_irq_reason = DEBUG
                ? 'end of ' + this.cmd_table[this.cmd_code].name + ' response'
                : ''
            this.msr &= ~MSR_RQM
            this.enter_command_phase()
...
FloppyController.prototype.start_read_write = function (args, do_write) {
    ...
    if (this.dor & DOR_DMAEN) {
        ...
        dbg_assert(
            false,
            this.cmd_table[this.cmd_code].name + ' in PIO mode not supported!',
        )
    }
}

FloppyController.prototype.end_read_write = function (
    status0,
    status1,
    status2,
) {
...
function FloppyDrive(name, fdd_config, buffer, fallback_drive_type) {
    ...
    dbg_log(
        'floppy drive ' + this.name + ' ready, drive type: ' + this.drive_type,
        LOG_FLOPPY,
    )
}
