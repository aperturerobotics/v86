...
V86.prototype.continue_init = async function (emulator, options) {
    ...
    if (settings.serial_console?.type === 'xtermjs') {
        const xterm_lib =
            settings.serial_console.xterm_lib || window['Terminal']
    ...
    const virtio_console_settings =
        options.virtio_console && typeof options.virtio_console === 'boolean'
            ? { type: 'none' }
            : options.virtio_console

    if (virtio_console_settings?.type === 'xtermjs') {
        const xterm_lib =
            virtio_console_settings.xterm_lib || window['Terminal']
            ...
            if (!settings.initial_state) {
                ...
                if (options.bzimage_initrd_from_filesystem) {
                    const { bzimage_path, initrd_path } =
                        this.get_bzimage_initrd_from_filesystem(settings.fs9p)
 ...
 */
V86.prototype.wait_until_vga_screen_contains = async function (
    expected,
    options,
) {
    ...
    const contains_expected = (screen_line, pattern) =>
        pattern.test
            ? pattern.test(screen_line)
            : screen_line.startsWith(pattern)
    const screen_lines = []
 ...
 */
V86.prototype.set_serial_container_xtermjs = function (
    element,
    xterm_lib = window['Terminal'],
) {
    this.serial_adapter &&
 ...
 * @param {Function} [xterm_lib]
 */
V86.prototype.set_virtio_console_container_xtermjs = function (
    element,
    xterm_lib = window['Terminal'],
) {
    this.virtio_console_adapter &&
