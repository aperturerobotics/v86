...
export function ScreenAdapter(options, screen_fill_buffer) {
    ...
    function render_font_bitmap(vga_bitmap) {
        ...
        if (
            ...
            font_image_data = font_context.createImageData(
                bitmap_width,
                bitmap_height,
            )
        }
    ...
    function render_changed_rows() {
        ...
        for (
            ...
            for (
                ...
                const chr_font_page =
                    chr_flags & FLAG_FONT_PAGE_B ? font_page_b : font_page_a
                const chr_visible =
                    (!(chr_flags & FLAG_BLINKING) || blink_visible) &&
                    font_is_visible[(chr_font_page << 8) + chr]
        ...
        if (n_rows_rendered) {
            if (blink_visible && cursor_enabled && changed_rows[cursor_row]) {
                const cursor_txt_i =
                    (cursor_row * text_mode_width + cursor_col) *
        ...
        if (mode === MODE_GRAPHICAL || mode === MODE_GRAPHICAL_TEXT) {
            ...
            for (let y = 0; y < text_mode_height; y++) {
                for (let x = 0; x < text_mode_width; x++) {
                    const index =
                        (y * text_mode_width + x) * TEXT_BUF_COMPONENT_SIZE
            ...
            if (
                ...
                cursor_row < text_mode_height &&
                cursor_col < text_mode_width
            ) {
    ...
    this.text_update_row = function (row) {
        ...
        for (var i = 0; i < text_mode_width; ) {
            ...
            while (
                i < text_mode_width &&
                (text_mode_data[offset + FLAGS_INDEX] & FLAG_BLINKING) ===
                    blinking &&
                ...
                if (row === cursor_row) {
                    if (i === cursor_col) {
                        ...
                        // found the cursor
                        cursor_element.style.backgroundColor =
    ...
    this.get_text_row = function (y) {
        const begin =
            y * text_mode_width * TEXT_BUF_COMPONENT_SIZE + CHARACTER_INDEX
