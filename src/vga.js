...
VGAScreen.prototype.vga_memory_read = function (addr) {
    if (this.svga_enabled) {
        ...
        return this.cpu.read8(
            (((addr - 0xa0000) | this.svga_bank_offset) + VGA_LFB_ADDRESS) | 0,
        )
    }
    ...
    if (this.planar_mode & 0x08) {
        ...
        if (this.color_dont_care & 0x1) {
            reading &=
                this.plane0[addr] ^ ~(this.color_compare & 0x1 ? 0xff : 0x00)
        }
        if (this.color_dont_care & 0x2) {
            reading &=
                this.plane1[addr] ^ ~(this.color_compare & 0x2 ? 0xff : 0x00)
        }
        if (this.color_dont_care & 0x4) {
            reading &=
                this.plane2[addr] ^ ~(this.color_compare & 0x4 ? 0xff : 0x00)
        }
        if (this.color_dont_care & 0x8) {
            reading &=
                this.plane3[addr] ^ ~(this.color_compare & 0x8 ? 0xff : 0x00)
        }
...
VGAScreen.prototype.set_size_graphical = function (
    width,
    height,
    virtual_width,
    virtual_height,
    ...
    switch (this.dispi_index) {
        ...
        case 8:
            ...
            if (this.svga_offset_x !== value) {
                ...
                this.svga_offset =
                    this.svga_offset_y * this.svga_width + this.svga_offset_x
                this.complete_redraw()
            }
            break
        case 9:
