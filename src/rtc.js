...
RTC.prototype.cmos_port_read = function () {
    ...
    switch (index) {
        ...
        case CMOS_RTC_HOURS:
            dbg_log(
                'read hour: ' +
                    h(this.encode_time(new Date(this.rtc_time).getUTCHours())),
                LOG_RTC,
            )
            // TODO: 12 hour mode
        ...
        case CMOS_RTC_DAY_MONTH:
            dbg_log(
                'read day of month: ' +
                    h(this.encode_time(new Date(this.rtc_time).getUTCDate())),
                LOG_RTC,
            )
...
RTC.prototype.cmos_port_write = function (data_byte) {
    switch (this.cmos_index) {
        ...
        default:
            dbg_log(
                'cmos write index ' + h(this.cmos_index) + ': ' + h(data_byte),
                LOG_RTC,
            )
    }
