            ...
            function () {
                ...
                if (flags & MULTIBOOT_HEADER_MEMORY_INFO) {
                    ...
                    for (
                        ...
                        if (
                            was_memory &&
                            cpu.memory_map_read8[addr >>> MMAP_BLOCK_BITS] !==
                                undefined
                        ) {
                        ...
                        if (program.type === 0) {
                            ...
                            if (
                                ...
                                if (
                                    ...
                                    program.vaddr + program.memsz > entrypoint
                                ) {
                                    entrypoint =
                                        entrypoint -
                                        program.vaddr +
...
CPU.prototype.dump_function_code = function (block_ptr, count) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        for (let i = start; i < end; i++) {
            buffer[i - start] = this.read8(
                this.translate_address_system_read(i),
            )
        }
...
CPU.prototype.get_regs_short = function () {
    ...
    for (var i = 0; i < 4; i++) {
        line1 +=
            r32_names[i] + '=' + h(this.reg32[r32[r32_names[i]]] >>> 0, 8) + ' '
        line2 +=
            r32_names[i + 4] +
            '=' +
        ...
        instructions.forEach(function (instr) {
            dbg_log(
                h(instr.address >>> 0) +
                    ': ' +
                    pads(
                        instr.bytes.map((x) => h(x, 2).slice(-2)).join(' '),
                        20,
                    ) +
                    ' ' +
                    instr.mnemonic +
