...
function IDEChannel(
    ...
    cpu.io.register_write(
        ...
        function (data) {
            ...
            this.current_interface.sector_count_reg =
                ((this.current_interface.sector_count_reg << 8) | data) & 0xffff
        },
    )
    ...
    cpu.io.register_write(
        ...
        function (data) {
            if (LOG_DETAILS & LOG_DETAIL_REG_IO) {
                dbg_log(
                    this.current_interface.name + ': write Command register',
                    LOG_DISK,
                )
            }
    ...
    cpu.io.register_read(
        this.control_base | ATA_REG_ALT_STATUS,
        this,
        this.read_status,
    )
...
IDEChannel.prototype.dma_set_addr = function (data) {
    if (LOG_DETAILS & LOG_DETAIL_RW_DMA) {
        dbg_log(
            this.current_interface.name + ': DMA set address: ' + h(data, 8),
            LOG_DISK,
        )
    }
    this.prdt_addr = data
...
IDEChannel.prototype.dma_write_command = function (value) {
    if (LOG_DETAILS & LOG_DETAIL_RW_DMA) {
        dbg_log(
            this.current_interface.name + ': DMA write command: ' + h(value),
            LOG_DISK,
        )
    }
...
IDEChannel.prototype.dma_write_command8 = function (value) {
    if (LOG_DETAILS & LOG_DETAIL_RW_DMA) {
        dbg_log(
            this.current_interface.name + ': DMA write command8: ' + h(value),
            LOG_DISK,
        )
    }
...
IDEInterface.prototype.set_disk_buffer = function (buffer) {
    ...
    if (this.interface_nr === 0) {
        ...
        const drive_reg =
            this.channel_nr === 0 ? CMOS_DISK_DRIVE1_CYL : CMOS_DISK_DRIVE2_CYL // 0x1B : 0x24 (drive C: or D:)
...
IDEInterface.prototype.atapi_check_condition_response = function (
    sense_key,
    additional_sense,
) {
...
IDEInterface.prototype.ata_advance = function (cmd, sectors) {
    ...
    } else // chs
    {
        ...
        this.head =
            (((new_sector / this.sectors_per_track) | 0) % this.head_count) &
            0xf
