...
function float_equal(x: number, y: number): boolean {
...
if ((cluster as any).isMaster) {
    function extract_json(
        ...
        if (!exception && fixture_text.includes('Program received signal')) {
            throw new Error(
                'Test was killed during execution by gdb: ' +
                    name +
                    '\n' +
                    fixture_text,
            )
        ...
        if (!regex_match || regex_match.length < 2) {
            throw new Error(
                'Could not find JSON in fixture text: ' +
                    fixture_text +
                    '\nTest: ' +
                    name,
            )
        ...
        const array = JSON.parse(regex_match[1])
        return {
            array: array,
            exception,
        }
    ...
    const tests = files.map((name) => {
        const fixture_name = name + '.fixture'
        const img_name = name + '.img'
        const fixture_text = fs.readFileSync(TEST_DIR + fixture_name)
        const fixture = extract_json(name, fixture_text)
    ...
    const failed_tests: any[] = []
    let finished_workers = 0

    for (let i = 0; i < nr_of_cpus; i++) {
        const worker = cluster.fork()

        worker.on('message', function (this: typeof worker, message: any) {
        ...
        worker.on('exit', function (code, _signal) {
            if (code !== 0 && code !== null) {
    ...
    function test_finished(): void {
        ...
        if (failed_tests.length > 0) {
            ...
            failed_tests.forEach(function (test_failure) {
                ...
                test_failure.failures.forEach(function (failure: any) {
                    console.error('\n\t' + failure.name)
    ...
    function run_test(test: any): void {
        if (!loaded) {
            first_test = test
            return
        }
    ...
    let current_test: any = undefined

    let first_test: any = undefined
    let waiting_to_receive_next_test = false
    let recorded_exceptions: { exception: string; eip: number }[] = []
    let test_timeout: ReturnType<typeof setTimeout>

    let emulator: any = new V86({
        autostart: false,
    ...
    emulator.cpu_exception_hook = function (n: number): boolean {
        ...
        emulator.v86.cpu.write32(
            emulator.v86.cpu.translate_address_system_read(eip),
            0xf4f4f4f4,
        ) // hlt
    ...
    function finish_test(): void {
        ...
        const evaluated_fpu_regs = new Float64Array(8).map((_, i) =>
            cpu.fpu_get_sti_f64(i),
        )
        const evaluated_mmxs = new Int32Array(16).map(
            (_, i) => cpu.fpu_st[((i & ~1) << 1) | (i & 1)],
        )
        const evaluated_xmms = cpu.reg_xmm32s
        const evaluated_memory = new Int32Array(
            cpu.mem8.buffer,
            cpu.mem8.byteOffset + BSS,
            (STACK_TOP - BSS) >> 2,
        )
        const _evaluated_fpu_tag = cpu.fpu_load_tag_word()
        const evaluated_fpu_status =
            cpu.fpu_load_status_word() & FPU_STATUS_MASK

        let individual_failures = []

        assert(current_test.fixture.array)

        const FLOAT_TRANSLATION: Record<string, number> = {
        ...
        let offset = 0
        const expected_reg32 = current_test.fixture.array.slice(
            offset,
            (offset += 8),
        )
        const expected_eip = current_test.fixture.array[offset++]
        const expected_fpu_regs = current_test.fixture.array
            .slice(offset, (offset += 8))
            .map((x: string) =>
                x in FLOAT_TRANSLATION ? FLOAT_TRANSLATION[x] : x,
            )
        const expected_mmx_registers = current_test.fixture.array.slice(
            offset,
            (offset += 16),
        )
        const expected_xmm_registers = current_test.fixture.array.slice(
            offset,
            (offset += 32),
        )
        const expected_memory = current_test.fixture.array.slice(
            offset,
            (offset += 8192 / 4),
        )
        const expected_eflags =
            current_test.fixture.array[offset++] & MASK_ARITH
        const fpu_tag = current_test.fixture.array[offset++]
        const fpu_status =
            current_test.fixture.array[offset++] & FPU_STATUS_MASK

        if (offset !== current_test.fixture.array.length) {
            throw new Error(
                'Bad fixture length in test ' + current_test.img_name,
            )
        }

        if (!current_test.fixture.exception) {
            for (let i = 0; i < cpu.reg32.length; i++) {
                const reg = cpu.reg32[i]
            ...
            if (fpu_tag !== FPU_TAG_ALL_INVALID) {
                for (let i = 0; i < evaluated_fpu_regs.length; i++) {
                    if (
                        expected_fpu_regs[i] !== 'invalid' &&
                        !float_equal(
                            evaluated_fpu_regs[i],
                            expected_fpu_regs[i],
                        )
                    ) {
                        individual_failures.push({
            ...
            for (let i = 0; i < evaluated_memory.length; i++) {
                if (evaluated_memory[i] !== expected_memory[i]) {
                    individual_failures.push({
                        name:
                            'mem[' +
                            (BSS + 4 * i).toString(16).toUpperCase() +
                            ']',
        ...
        individual_failures = individual_failures.map(
    ...
    cluster.worker!.on('message', function (message: any) {
