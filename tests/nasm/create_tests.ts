...
async function make_test(test: any): Promise<void> {
    if (LOG_VERBOSE) console.log('Start', test.name || test.file)
    let asm_file
    let img_file
    let tmp_file
    ...
    if (test.asm) {
        ...
        if (old_code === test.asm) {
            if (LOG_VERBOSE) console.log('Skip', test.name || test.file)
            return
        }
        ...
        try {
            if (
                (await fse.stat(asm_file)).mtime <
                (await fse.stat(img_file)).mtime
            ) {
                return
            }
        } catch (e) {
    ...
    if (LOG_VERBOSE) {
        console.log(
            'nasm',
            ['-w+error', '-felf32', '-o', tmp_file, asm_file].join(' '),
        )
    }
    await execFile(
        'nasm',
        ['-w+error', '-felf32', '-o', tmp_file, asm_file],
        options,
    )
    if (LOG_VERBOSE) {
        console.log(
            ...
            [
                ...
                '-m',
                'elf_i386',
                '--section-start=.bss=0x100000',
                '--section-start=.text=0x80000',
                '--section-start=.multiboot=0x20000',
                '-o',
                img_file,
            ].join(' '),
        )
...
function create_tests(): any[] {
    const tests: any[] = []

    const asm_files = fs
        .readdirSync(__dirname)
        .filter((f) => f.endsWith('.asm'))
    tests.push(...asm_files.map((file) => ({ file })))
    ...
    for (const op of encodings) {
        ...
        for (const config of configurations) {
            for (let nth_test = 0; nth_test < NUMBER_TESTS; nth_test++) {
                ...
                for (const asm of create_instruction_test(
                    op,
                    config,
                    nth_test,
                )) {
...
function format_opcode(n: number): string {
    const x = n.toString(16)
    return x.length === 1 || x.length === 3 ? '0' + x : x
}

function create_nasm_modrm_combinations_16(): string[][] {
    const result = []
    ...
    for (let modrm = 0; modrm < 0xc0; modrm++) {
        ...
        const has_imm8 = mod === 1
        const has_imm16 = mod === 2 || (rm === 6 && mod === 0)
...
function create_nasm_modrm_combinations_32(): string[][] {
    const result = []

    const sample_sib_bytes = [0x05, 0x65, 0xad, 0xcd, 0x20, 0xff]
    const exhaustive_sib_bytes = []
    ...
    for (let modrm = 0; modrm < 0xc0; modrm++) {
        ...
        const has_imm8 = mod === 1
        const has_imm32 = mod === 2 || (rm === 5 && mod === 0)
        const has_sib = rm === 4
        ...
        if (has_sib) {
            ...
            const sib_bytes =
                reg === 0 ? exhaustive_sib_bytes : sample_sib_bytes

            for (const sib of sib_bytes) {
                const line = ['db ' + modrm, 'db ' + sib]
                if (has_imm8) line.push('db 9ah')
                if (has_imm32 || (mod === 0 && (sib & 7) === 5))
                    line.push('dd 9a1fbcdeh')
                result.push(line)
            }
        } else {
            const line = ['db ' + modrm]
...
function rand_reg_but_not_esp(rng: any): number {
    const r = rng.int32() & 7
    return r === 4 ? rand_reg_but_not_esp(rng) : r
}

function interesting_immediate(rng: any): number {
    if (rng.int32() & 1) {
        return rng.int32()
    } else {
...
function create_instruction_test(
    op: Encoding,
    config: { mem: number; size: number },
    nth_test: number,
): any[] {
    ...
    const rng = Rand(1283698341 ^ (op.opcode + nth_test * 0x10000))
    ...
    for (const reg of ['eax', 'ecx', 'edx', 'ebx', 'ebp', 'esi', 'edi']) {
        const rand = rng.int32()
    ...
    if (opcode === 0x8d) {
        ...
        codes.push(
            ([] as string[]).concat(
                create_nasm_modrm_combinations_16().map((lines) =>
                    ['db 67h', 'db 8dh'].concat(lines).join('\n'),
                ),
                create_nasm_modrm_combinations_32().map((lines) =>
                    ['db 8dh'].concat(lines).join('\n'),
                ),
            ),
        )
    } else {
        ...
        if (opcode >= 0x100) {
            const c = opcode >> 8
            assert(
                c === 0x0f || c === 0xf2 || c === 0xf3,
                'Expected 0F, F2, or F3 prefix, got ' + c.toString(16),
            )
            codes.push('db ' + c)
            opcode &= ~0xff00
        }
...
function create_arith_test(i: number): { name: string; asm: string } {
    const rng = Rand(916237867 ^ i)

    const registers_by_size: Record<number, string[]> = {
        8: ['al', 'ah', 'cl', 'ch', 'dl', 'dh', 'bl', 'bh'],
        16: ['ax', 'cx', 'dx', 'bx', 'sp', 'bp', 'si', 'di'],
        32: ['eax', 'ecx', 'edx', 'ebx', 'esp', 'ebp', 'esi', 'edi'],
    }
    const mask_by_size: Record<number, number> = {
        8: 0xff,
        16: 0xffff,
        32: -1,
    }
    const word_by_size: Record<number, string> = {
        8: 'byte',
        16: 'word',
        32: 'dword',
    }
    const two_operand_instructions = [
        'add',
        'sub',
        'adc',
        'sbb',
    ...
    for (const reg of registers_by_size[32]) {
        if (reg !== 'esp') {
    ...
    for (let i = 0; i < 2000; i++) {
        ...
        if (
            ins === 'imul' &&
            rng.int32() & 1
        ) // other encodings handled in one_operand_instructions
        {
            ...
            if (rng.int32() & 1) {
                ...
                const src2 = nasm_hex(
                    interesting_immediate(rng) & mask_by_size[size_imul],
                )
        ...
        if (dst_is_mem || src_is_mem) {
            if (rng.int32() & 1) {
                ...
                c.push(
                    `mov dword [${nasm_hex(address)}], ${nasm_hex(interesting_immediate(rng) & 0xff)}`,
                )
            }
        }
        ...
        // Random branch selection for setcc, cmovcc, pushf, or nothing.
        // Uses rng.int32() & 3 to select a branch (0-3) with ~equal probability,
        // consuming a single RNG call instead of the original cascading calls.
        const rng_branch = rng.int32() & 3
        if (rng_branch === 0) {
            // setcc
            const cond = random_pick(
                conditions
                    .filter(([_, flag]) => 0 === (flag & undefined_flags))
                    .map(([suffix]) => suffix),
                rng,
            )
            assert(cond)
            ...
            const dst2 =
                rng.int32() & 1
                    ? `byte [${nasm_hex(address++)}]`
                    : registers_by_size[8][rng.int32() & 7]
            c.push(`${ins2} ${dst2}`)
        } else if (rng_branch === 1) {
            // cmovcc
            const cond = random_pick(
                conditions
                    .filter(([_, flag]) => 0 === (flag & undefined_flags))
        ...
        } else if (rng_branch === 2) {
            c.push('pushf')
            c.push(
                'and dword [esp], ~' + nasm_hex(FLAGS_IGNORE | undefined_flags),
            )
...
function all_combinations(xs: any[]): any[][] {
    let result = [xs]
...
function nasm_hex(x: number): string {
    return `0${(x >>> 0).toString(16).toUpperCase()}h`
}

function random_pick<T>(xs: T[], rng: any): T {
