...
function chunk(source: string[], num_chunks: number): string[][] {
...
if (SYNC_GDB_EXECUTION) {
    ...
    console.log(
        '[+] Using %d cpus to generate %d fixtures',
        nr_of_cpus,
        test_files.length,
    )
}
...
function test_arg_formatter(workload: string[]): string[] {
...
function set_proc_handlers(proc: any, n: number): void {
    proc.on('close', (code: number) => on_proc_close(code, n))
...
function on_proc_close(code: number | null, n: number): void {
