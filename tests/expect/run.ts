...
// @ts-expect-error no type declarations for build artifact
...
function run_all(): void {
    ...
    const files: TestFile[] = asm_files
        .map((asm_file) => {
            const name = asm_file.slice(0, -4)
            return {
                name,
                expect_file: path.relative(
                    '.',
                    path.join(TEST_DIR, name + '.wast'),
                ),
                actual_file: path.relative(
                    '.',
                    path.join(BUILD_DIR, name + '.actual.wast'),
                ),
                actual_wasm: path.relative(
                    '.',
                    path.join(BUILD_DIR, name + '.wasm'),
                ),
    ...
    function next_test(i: number): void {
        if (files[i]) {
...
function normalise_wast(wast: string): string {
    return wast
        .replace(/offset=(\d+)/g, function (match, offset) {
...
function run_test(
    ...
    emulator.add_listener('emulator-loaded', function () {
        ...
        cpu.test_hook_did_generate_wasm = function (wasm: Uint8Array) {
            ...
            cpu.test_hook_did_generate_wasm = function () {
                cpu.test_hook_did_generate_wasm = function () {}
                throw new Error(
                    'Hook for wasm generation called multiple times',
                )
            }

            let expect_file_for_diff: string
            if (!fs.existsSync(expect_file)) {
                // enhanced workflow: If file doesn't exist yet print full diff
                expect_file_for_diff = '/dev/null'
            } else {
...
function disassemble_wasm(wasm: Uint8Array): string {
    ...
    let module: any
    try {
        module = libwabt.readWasm(wasm, { readDebugNames: false })
        module.generateNames()
        module.applyNames()
        return module.toText({ foldExprs: true, inlineExport: true })
    } catch (e) {
        console.error('Error while running libwabt: ' + (e as Error).toString())
