...
fs.copyFileSync(
    path.join(__dirname, '/test-i386'),
    path.join(share_dir_9p, '/test-i386'),
)

const qemu_version = spawnSync(QEMU, ['--version'])
assert(
    qemu_version.status === 0,
    'QEMU not found, return code: ' + qemu_version.status,
)
...
const qemu = spawn(QEMU, [
    ...
    'stdio',
    '-cdrom',
    path.join(__dirname, '/../../images/linux4.iso'),
    '-device',
    'virtio-9p-pci,fsdev=fs9p,mount_tag=host9p',
    '-fsdev',
    `local,id=fs9p,path=${share_dir_9p},security_model=none`,
    '-display',
    'none',
    '-cpu',
    'Westmere', // default doesn't support popcnt
...
qemu.stdout.on('data', (data: Buffer) => {
...
qemu.stderr.on('data', (data: Buffer) => {
    process.stderr.write(data)
})
