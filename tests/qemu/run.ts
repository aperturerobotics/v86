...
const test_executable = new Uint8Array(
    fs.readFileSync(__dirname + '/test-i386'),
)
...
let outfile: { write(buffer: Buffer): void } = process.stdout
if (process.argv[2]) {
    const fh = await fs.promises.open(process.argv[2], 'w')
    outfile = {
        write(buffer: Buffer) {
            fh.write(buffer)
        },
...
emulator.add_listener('serial0-output-byte', async function (byte: number) {
