...
const { V86 } = await import(
    TEST_RELEASE_BUILD ? '../../dist/v86.js' : '../../src/main.js'
)
...
function get_line(screen: Uint8Array, y: number): Uint8Array {
    return screen.subarray(y * SCREEN_WIDTH, (y + 1) * SCREEN_WIDTH)
}

function line_to_text(screen: Uint8Array, y: number): string {
    return bytearray_to_string(get_line(screen, y))
}

function string_to_bytearray(str: string): Uint8Array {
    return new Uint8Array(str.split('').map((chr) => chr.charCodeAt(0)))
}

function bytearray_to_string(arr: Uint8Array): string {
...
function screen_to_text(s: Uint8Array): string {
    const result: string[] = []
    result.push(
        '+==================================== SCREEN ====================================+',
    )
...
if ((cluster as any).isPrimary) {
    let tests: any[] = [
        {
            ...
            timeout: 20,
            expected_texts: ['Welcome to FreeDOS'],
        },
        {
        ...
        },
        {
            ...
            timeout: 90,
            expected_texts: ['/root%', 'test passed'],
            actions: [
                {
                    on_text: '/root%',
        ...
        },
        {
            ...
            cdrom:
                root_path +
                '/images/experimental/en_windows_vista_sp2_x86_dvd_342266.iso',
        ...
        },
        {
            ...
            hda:
                root_path +
                '/images/experimental/copy_winxp_lite-from-pixelsuft.img',
            memory_size: 512 * 1024 * 1024,
            timeout: 300,
            expect_graphical_mode: true,
            expect_graphical_size: [800, 600],
        ...
        },
        {
            ...
            timeout: 200,
            expected_texts: ['~%'],
            expected_serial_text: ['Files send via emulator appear in'],
            expect_mouse_registered: true,
        },
        {
            name: 'Linux bzImage',
        ...
        },
        {
            ...
            expect_graphical_size: [1024, 768],
            expect_mouse_registered: true,
            expected_texts: ['MODE prepare code page function completed'],
        },
        {
            name: 'FreeBSD',
            skip_if_disk_image_missing: true,
            ...
            expected_texts: ['FreeBSD/i386 (nyu) (ttyv0)', 'root@nyu:~ #'],
            actions: [
                {
                    on_text: '   Autoboot in',
                    run: '\n',
        ...
        },
        {
            ...
            cdrom:
                root_path +
                '/images/experimental/os/FreeBSD-11.0-RELEASE-i386-bootonly.iso',
        ...
        },
        {
            ...
            actions: [
                {
                    after: 1000,
                    run: 'ls --color=never /dev/ /usr/bin/ > /dev/ttyS0\n',
                },
                {
                    after: 2000,
                    run: `python -c 'print(100 * "a")' > /dev/ttyS0\n`,
                },
            ],
        ...
        },
        {
            ...
            hda:
                root_path +
                '/images/haiku-r1beta2-hrev54154_111-x86_gcc2h-anyboot.iso',
            expected_serial_text: [
                'init_hardware()',
        ...
        },
        {
            ...
            hda:
                root_path +
                '/images/reactos-livecd-0.4.15-dev-73-g03c09c9-x86-gcc-lin-dbg.iso',
            expect_graphical_mode: true,
            expect_graphical_size: [800, 600],
            expect_mouse_registered: true,
            actions: [
        ...
        },
        {
            ...
            hda:
                root_path +
                '/images/redox_demo_i686_2022-11-26_643_harddrive.img',
            actions: [
        ...
        },
        {
            ...
            fda:
                root_path +
                '/images/experimental/freedos-fds/freedos.boot.disk.160K.img',
            timeout: 10,
            expected_texts: ['A:\\>'],
        },
        {
            ...
            fda:
                root_path +
                '/image/experimentals/freedos-fds/freedos.boot.disk.180K.img',
            timeout: 10,
            expected_texts: ['A:\\>'],
        },
        {
            ...
            fda:
                root_path +
                '/image/experimentals/freedos-fds/freedos.boot.disk.320K.img',
            timeout: 10,
            expected_texts: ['A:\\>'],
        },
        {
            ...
            fda:
                root_path +
                '/image/experimentals/freedos-fds/freedos.boot.disk.360K.img',
            timeout: 10,
            expected_texts: ['A:\\>'],
        },
        {
            ...
            fda:
                root_path +
                '/image/experimentals/freedos-fds/freedos.boot.disk.640K.img',
            timeout: 10,
            expected_texts: ['A:\\>'],
        },
        {
            ...
            fda:
                root_path +
                '/image/experimentals/freedos-fds/freedos.boot.disk.1200K.img',
            timeout: 10,
            expected_texts: ['A:\\>'],
        },
        {
        ...
        },
        {
            ...
            acpi: true,
            expected_serial_text: ['Welcome to Crazierl:'],
        },
        {
            name: 'Fiwix',
            skip_if_disk_image_missing: true,
            ...
            expect_mouse_registered: true,
            expected_texts: ['(root):~#'],
            actions: [{ on_text: '(root):~#', run: '/usr/games/lsdoom\n' }],
        },
        {
            name: '9legacy',
            use_small_bios: true, // has issues with 256k bios
            ...
            expect_graphical_mode: true,
            expect_mouse_registered: true,
            expected_texts: ['Selection:'],
            actions: [{ on_text: 'Selection:', run: '1\n' }],
            expected_serial_text: ['init: starting'],
        },
        {
            name: 'BSD/OS 3',
            skip_if_disk_image_missing: true,
        ...
        },
        {
            ...
            actions: [
                {
                    on_text: '                   BIOS default device boot in',
                    run: '\n',
                    after: 5000,
                },
            ],
        ...
        },
        {
            ...
            actions: [
                ...
                {
                    on_text: 'tc@box',
                    run: 'sudo mount /dev/fd0 /mnt && ls /mnt\n',
                },
            ],
        ...
        },
        {
            ...
            actions: [
                ...
                {
                    on_text: 'tc@box',
                    run: 'sudo mount /dev/sda1 /mnt && ls /mnt\n',
                },
            ],
    ...
    function send_work_to_worker(worker: any, _message: any): void {
    ...
    for (let i = 0; i < nr_of_cpus; i++) {
        ...
        worker.on(
            'exit',
            function (code: number | null, signal: string | null) {
                if (signal) {
    ...
    cluster.worker!.on('message', function (test_case: any) {
...
function bytearray_starts_with(arr: Uint8Array, search: Uint8Array): boolean {
...
function run_test(test: any, done: () => void): void {
    ...
    const images = [
        ...
        test.bzimage,
        test.multiboot,
        test.filesystem && test.filesystem.basefs,
    ].filter((x) => x)
    ...
    if (test.alternative_bios) {
        bios = root_path + '/bios/bochs-bios.bin'
        vga_bios = root_path + '/bios/bochs-vgabios.bin'
    } else if (test.use_small_bios || TEST_RELEASE_BUILD) {
        bios = root_path + '/bios/seabios.bin'
        vga_bios = root_path + '/bios/vgabios.bin'
    } else {
        bios = root_path + '/bios/seabios-debug.bin'
        vga_bios = root_path + '/bios/vgabios-debug.bin'
    }
    ...
    settings.bzimage_initrd_from_filesystem =
        test.bzimage_initrd_from_filesystem
    settings.acpi =
        test.acpi === undefined && !test.state ? TEST_ACPI : test.acpi
    ...
    const screen = new Uint8Array(SCREEN_WIDTH * 25)

    function check_text_test_done(): boolean {
        return test.expected_texts.length === 0
    }

    function check_serial_test_done(): boolean {
        return test.expected_serial_text.length === 0
    }

    let mouse_test_done = false
    function check_mouse_test_done(): boolean {
        return !test.expect_mouse_registered || mouse_test_done
    }

    let graphical_test_done = false
    let size_test_done = false
    function check_graphical_test_done(): boolean {
        return (
            !test.expect_graphical_mode ||
            (graphical_test_done &&
                (!test.expect_graphical_size || size_test_done))
        )
    }

    const test_start = Date.now()

    const timeout_seconds = test.timeout * TIMEOUT_EXTRA_FACTOR
    const timeout = setTimeout(check_test_done, (timeout_seconds + 1) * 1000)
    const timeouts: ReturnType<typeof setTimeout>[] = [timeout]

    const on_text: any[] = []
    let stopped = false

    let screen_interval: ReturnType<typeof setInterval> | null = null

    function check_test_done(): void {
        if (stopped) {
            return
        }

        if (
            check_text_test_done() &&
            ...
            if (!check_text_test_done()) {
                console.warn(
                    'Expected text "%s" after %d seconds.',
                    bytearray_to_string(test.expected_texts[0]),
                    timeout_seconds,
                )
            }
            ...
            if (!check_graphical_test_done()) {
                console.warn(
                    'Expected graphical mode after %d seconds.',
                    timeout_seconds,
                )
            }
            ...
            if (!check_mouse_test_done()) {
                console.warn(
                    'Expected mouse activation after %d seconds.',
                    timeout_seconds,
                )
            }
            ...
            if (!check_serial_test_done()) {
                console.warn(
                    'Expected serial text "%s" after %d seconds.',
                    test.expected_serial_text,
                    timeout_seconds,
                )
            }
            ...
            if (on_text.length) {
                console.warn(
                    `Note: Expected text "${bytearray_to_string(on_text[0].text)}" to run "${on_text[0].run}"`,
                )
            }
    ...
    emulator.add_listener('screen-set-size', function (args: number[]) {
        const [w, h, bpp] = args
        graphical_test_done = bpp !== 0

        if (test.expect_graphical_size) {
            size_test_done =
                w === test.expect_graphical_size[0] &&
                h === test.expect_graphical_size[1]
        }

        check_test_done()
    })

    emulator.add_listener('screen-put-char', function (chr: number[]) {
        const y = chr[0]
        const x = chr[1]
        const code = chr[2]
        screen[x + SCREEN_WIDTH * y] = code

        const line = get_line(screen, y)

        if (!check_text_test_done()) {
            const expected = test.expected_texts[0]
            if (x < expected.length && bytearray_starts_with(line, expected)) {
                test.expected_texts.shift()
                if (VERBOSE)
                    console.log(
                        `Passed: "${bytearray_to_string(expected)}" on screen (${test.name})`,
                    )
                check_test_done()
            }
        }
        ...
        if (on_text.length) {
            const expected = on_text[0].text

            if (x < expected.length && bytearray_starts_with(line, expected)) {
                const action = on_text.shift()

                timeouts.push(
                    setTimeout(() => {
                        if (VERBOSE) console.error("Sending '%s'", action.run)
                        if (typeof action.run[0] === 'string')
                            emulator.keyboard_send_text(action.run, 10)
        ...
        if (c === '\n') {
            ...
            if (test.expected_serial_text.length) {
                ...
                if (serial_line.includes(expected)) {
                    ...
                    if (VERBOSE)
                        console.log(
                            `Passed: "${expected}" on serial (${test.name})`,
                        )
                    check_test_done()
                }
            }
    ...
    if (test.actions) {
        test.actions.forEach(function (action: any) {
            if (action.on_text) {
                on_text.push({
                    text: string_to_bytearray(action.on_text),
                    run: action.run,
                    after: action.after,
                })
            } else {
