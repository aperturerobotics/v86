...
if (isMainThread) {
    ...
    const tests: TestCase[] = [
        {
            ...
            end_trigger: 'done\tudhcpc',
                ...
                assert(
                    /lease of 192.168.86.100 obtained/.test(capture),
                    'lease of 192.168.86.100 obtained',
                )
            },
        },
        {
        ...
        },
        {
            ...
            end_trigger: 'done\tping',
                ...
                assert(
                    /2 packets transmitted, 2 (packets )?received, 0% packet loss/.test(
                        capture,
                    ),
                    '2 packets transmitted, 2 packets received, 0% packet loss',
                )
                assert(
                    /from 1\.2\.3\.4:/.test(capture),
                    'got correct source ip',
                )
            },
        },
        {
            ...
            end_trigger: 'done\tarp',
                ...
                assert(
                    /.192.168.86.1. at 52:54:00:01:02:03 \[ether\] {2}on eth0/.test(
                        capture,
                    ),
                    '(192.168.86.1) at 52:54:00:01:02:03 [ether]  on eth0',
                )
            },
        },
        {
            ...
            start: async () => {
                ...
                emulator.serial0_send(
                    'echo -n hello | socat TCP4-LISTEN:80 - && echo -e done\\\\tlisten\n',
                )
                await wait(1000)
                open = await emulator.network_adapter.tcp_probe(80)
                assert(
                    open,
                    'Probe shows port open, but does not show as a connection',
                )
                await wait(1000)
                const h = emulator.network_adapter.connect(80)
                h.on('connect', () => {
                    h.write(new TextEncoder().encode('From VM: '))
                    h.on('data', (d: Uint8Array) => {
                        d.reverse()
                        h.write(d)
        ...
        },
        {
            ...
            end_trigger: 'done\tmocked.example.org',
                ...
                assert(
                    /This text is from the mock/.test(capture),
                    'got mocked.example.org text',
                )
            },
        },
        {
            ...
            end_trigger: 'done\texample.org',
                ...
                assert(
                    /This domain is for use in documentation examples without needing permission/.test(
                        capture,
                    ),
                    'got example.org text',
                )
            },
        },
        {
            ...
            start: () => {
                emulator.serial0_send(
                    `wget -T 10 -O - ${SERVER_PORT}.external\n`,
                )
            ...
            end_trigger: 'done\tlocal server',
                ...
                assert(
                    /This text is from the local server/.test(capture),
                    'got local server text',
                )
            },
        },
        {
            ...
            start: () => {
                emulator.serial0_send(
                    `wget -S -T 10 --header='x-client-test: hello' -O - ${SERVER_PORT}.external/header\n`,
                )
                emulator.serial0_send(
                    'echo -e done\\\\tlocal server custom header\n',
                )
            },
            end_trigger: 'done\tlocal server custom header',
            end: (capture) => {
                assert(
                    /x-server-test: {1,}h_e_l_l_o/.test(capture),
                    'got local server custom header',
                )
            },
        },
        {
            ...
            start: () => {
                emulator.serial0_send(
                    `curl -m 10 -L -v ${SERVER_PORT}.external/redirect\n`,
                )
                emulator.serial0_send(
                    'echo -e done\\\\tlocal server redirect\n',
                )
            },
            end_trigger: 'done\tlocal server redirect',
            end: (capture) => {
                assert(
                    /x-was-fetch-redirected: {1,}true/.test(capture),
                    'got local server redirect header',
                )
                assert(
                    /This text is from the local server/.test(capture),
                    'got actual redirect',
        ...
        },
        {
            ...
            start: () => {
                emulator.serial0_send(
                    "wget --header='test.v86: 123' -T 10 -O - test.domain\n",
                )
                emulator.serial0_send(
                    'echo -e done\\\\tincorrect header name\n',
                )
            },
        ...
        },
        {
            ...
            start: () => {
                emulator.serial0_send(
                    "wget --header='test:' -T 10 -O - test.domain\n",
                )
        ...
        },
        {
            ...
            start: () => {
                emulator.serial0_send(
                    "wget --header='testheader' -T 10 -O - test.domain\n",
                )
    ...
    server.on('message', function (msg: number) {
        SERVER_PORT = msg
        console.log('Server started on port ' + SERVER_PORT)
    })

    emulator.add_listener('emulator-ready', function () {
        const network_adapter = emulator.network_adapter
        const original_fetch = network_adapter.fetch
        network_adapter.fetch = (fetchUrl: string, opts: RequestInit) => {
            if (/^http:\/\/mocked.example.org\/?/.test(fetchUrl)) {
                const contents = new TextEncoder().encode(
                    'This text is from the mock',
                )
                const headers = new Headers()
                headers.append('Content-Type', 'text/plain')
                headers.append('Content-Length', String(contents.length))
                return new Promise((res: (value: Response) => void) =>
                    setTimeout(
                        () =>
                            res(
            ...
            return original_fetch(fetchUrl, opts)
        }
    })
    ...
    let line = ''
    let capture = ''
    let end_trigger: string | undefined
            ...
            case 'GET':
                if (request.url === '/') {
                    ...
                    response.writeHead(200, {
                        'x-server-test':
                            (request.headers['x-client-test'] as string)
                                .split('')
                                .join('_') || 'none',
                    })
                    response.end()
    ...
    server.listen(0, () =>
        parentPort!.postMessage((server.address() as { port: number }).port),
    )
}
