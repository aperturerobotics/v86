...
interface TestCase {
    ...
    end_trigger: string
    end: (capture: string) => void | Promise<void>
}
...
const tests: TestCase[] = [
    {
        ...
        start: async () => {
            ...
            emulator.serial0_send(
                'dd if=/dev/urandom of=/tmp/file bs=1M count=64\n',
            )
    ...
    },
    {
        ...
        start: async () => {
            ...
            emulator.v86.cpu.devices.virtio_balloon.GetStats((s: unknown) => {
                console.log(s)
            })
            emulator.v86.cpu.devices.virtio_balloon.Cleanup(
                async (zeroed: number) => {
        ...
        end_trigger: 'done',
            ...
            const s2: ArrayBuffer = await emulator.save_state()
            const saved = s1.byteLength - s2.byteLength
            console.log(
                `Saved ${saved >> 20}MB of ${s1.byteLength >> 20}MB (${(saved / s1.byteLength) * 100}%)`,
            )
    ...
    },
    {
        ...
        end_trigger: 'done',
            ...
            assert(
                emulator.v86.cpu.devices.virtio_balloon.actual === 30000,
                'Got 30000 pages from vm',
            )
        },
    },
]
