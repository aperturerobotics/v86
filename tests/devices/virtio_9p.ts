...
const testfsjson = JSON.parse(
    fs.readFileSync(__dirname + '/testfs.json', 'utf-8'),
)
const SHOW_LOGS = false
const STOP_ON_FIRST_FAILURE = false

function assert_equal(actual: any, expected: any, message?: string): void {
    if (actual !== expected) {
        console.warn(
            'Failed assert equal (Test: %s). %s',
            tests[test_num].name,
            message || '',
        )
...
function assert_not_equal(actual: any, expected: any, message?: string): void {
    if (actual === expected) {
        console.warn(
            'Failed assert not equal (Test: %s). %s',
            tests[test_num].name,
            message || '',
        )
...
const test_file = new Uint8Array(512).map((_v) => 0x20 + Math.random() * 0x5e)
const test_file_string = Buffer.from(test_file).toString()
const test_file_small = new Uint8Array(16).map(
    (_v) => 0x20 + Math.random() * 0x5e,
)
...
const tests = [
    {
        ...
        start: () => {
            ...
            emulator.serial0_send(
                'echo start-capture; cat /etc/profile; echo done-read-existing\n',
            )
        },
        capture_trigger: 'start-capture',
        end_trigger: 'done-read-existing',
        end: async (capture: string, done: () => void) => {
    ...
    },
    {
        ...
        start: () => {
            emulator.serial0_send(
                'dd if=/dev/zero of=/mnt/read-new bs=1k count=512\n',
            )
    ...
    },
    {
        ...
        start: () => {
            emulator.serial0_send(
                'echo start-capture; cat /mnt/write-new; echo; echo done-write-new\n',
            )
        },
        capture_trigger: 'start-capture',
        end_trigger: 'done-write-new',
        end: (capture: string, done: () => void) => {
    ...
    },
    {
        ...
        start: () => {
            emulator.serial0_send(
                'echo start-capture; echo foo > /mnt/bar; ls  -l --full-time --color=never /mnt/bar; echo; echo done-write-new\n',
            )
        },
        capture_trigger: 'start-capture',
        end_trigger: 'done-write-new',
            ...
            const outputs = capture
                .split('\n')
                .map((output: string) => output.split(/\s+/))

            // atime: Should be fresh
            const [year, _month, _day] = outputs[0][5].split('-')
    ...
    },
    {
        ...
        start: () => {
            ...
            emulator.serial0_send(
                'mv /mnt/thirddir/otherdir /root/movedoutside;',
            )
    ...
    },
    {
        ...
        start: () => {
            ...
            emulator.serial0_send(
                'test ! -e /mnt/new-file && echo new-file-unlinked;',
            )
            emulator.serial0_send(
                'cat /mnt/new-file 2>/dev/null || echo read-failed;',
            )

            emulator.serial0_send('rm /mnt/existing-file;')
            emulator.serial0_send(
                'test ! -e /mnt/existing-file && echo existing-file-unlinked;',
            )
            emulator.serial0_send(
                'cat /mnt/existing-file 2>/dev/null || echo read-failed;',
            )

            emulator.serial0_send(
                'rmdir /mnt/new-dir 2>/dev/null || echo rmdir-failed;',
            )
            ...
            emulator.serial0_send(
                'test ! -e /mnt/new-dir/file && echo new-dir-file-unlinked;',
            )
            emulator.serial0_send(
                'test ! -e /mnt/new-dir && echo new-dir-unlinked;',
            )
            emulator.serial0_send(
                'ls /mnt/new-dir 2>/dev/null || echo read-failed;',
            )
    ...
    },
    {
        ...
        start: () => {
            ...
            emulator.serial0_send(
                '{ test /mnt/link-other -ef /mnt/dir/link3 || \n',
            )
            emulator.serial0_send(
                '  echo different inode | tee -a /mnt/link1; }\n',
            )
            ...
            emulator.serial0_send(
                'nlinks /mnt/target | tee -a /mnt/dir/link2\n',
            )
            emulator.serial0_send(
                'nlinks /mnt/dir/link2 | tee -a /mnt/dir/link2\n',
            )
            emulator.serial0_send(
                'nlinks /mnt/target2 | tee -a /mnt/dir/link2\n',
            )
            emulator.serial0_send(
                'nlinks /mnt/link-other | tee -a /mnt/dir/link2\n',
            )
            ...
            emulator.serial0_send(
                'echo unlinked original | tee -a /mnt/dir/link3\n',
            )

            // Test inode numbers after movement and unlinking.
            emulator.serial0_send(
                '{ test /mnt/link1-renamed -ef /mnt/link2-moved && \n',
            )
            emulator.serial0_send(
                '  test /mnt/link2-moved -ef /mnt/dir/link3 && \n',
            )
            emulator.serial0_send(
                '  echo same inode after mv | tee -a /mnt/link1-renamed; }\n',
            )
            ...
            emulator.serial0_send(
                'nlinks /mnt/link1-renamed | tee -a /mnt/link2-moved\n',
            )
            ...
            emulator.serial0_send(
                'echo unlinked link1 >> /mnt/link2-moved;\\\n',
            )
            emulator.serial0_send(
                'nlinks /mnt/link2-moved >> /mnt/link2-moved;\\\n',
            )
            emulator.serial0_send('rm /mnt/link2-moved;\\\n')
            emulator.serial0_send('echo unlinked link2 >> /mnt/dir/link3;\\\n')
            emulator.serial0_send(
                'nlinks /mnt/dir/link3 >> /mnt/dir/link3;\\\n',
            )
    ...
    },
    {
        ...
        start: () => {
            ...
            emulator.serial0_send(
                '(cat /mnt/testfifo > /mnt/testfifo-output;echo "\ndone-fifo") &\n',
            )
    ...
    },
    {
        ...
        start: () => {
            ...
            emulator.serial0_send(
                'dd if=/dev/zero of=/mnt/file bs=1k count=4 status=none;',
            )
            emulator.serial0_send('df -PTk /mnt | tail -n 1;')
    ...
    },
    {
        ...
        start: () => {
            ...
            emulator.serial0_send(
                'dd if=/dev/zero of=/mnt/file bs=1 count=137 status=none;',
            )
    ...
    },
    {
        ...
        start: () => {
            ...
            emulator.serial0_send(
                'setfattr --name=user.attr1 --value="val1" /mnt/file;',
            )
            emulator.serial0_send(
                'setfattr --name=user.attr2 --value="val2" /mnt/file;',
            )
            emulator.serial0_send(
                'setfattr --name=user.mime_type --value="text/plain" /mnt/file;',
            )
            emulator.serial0_send(
                'setfattr --name=user.nested.attr --value="foobar" /mnt/file;',
            )

            // Unrecognized attribute name under other namespaces should be allowed.
            emulator.serial0_send(
                'setfattr --name=security.not_an_attr --value="val3" /mnt/file;',
            )

            // Remove the caps attribute we've automatically put in. Tested later.
            emulator.serial0_send(
                'setfattr --remove=security.capability /mnt/file;',
            )

            emulator.serial0_send(
                'getfattr --encoding=text --absolute-names --dump /mnt/file | sort;',
            )
            emulator.serial0_send(
                'getfattr --encoding=text --absolute-names --name=user.nested.attr /mnt/file;',
            )
            emulator.serial0_send(
                'getfattr --encoding=text --absolute-names --name=security.not_an_attr /mnt/file;',
            )
            emulator.serial0_send(
                'getfattr --encoding=text --absolute-names --name=user.attr2 /mnt/file;',
            )
    ...
    },
    {
        ...
        start: () => {
            ...
            emulator.serial0_send(
                'setfattr --name=user.foo --value=bar /mnt/file;',
            )
            // File contents should not be overriden.
            emulator.serial0_send('cat /mnt/file;')
            emulator.serial0_send(
                'getfattr --encoding=hex --absolute-names --name=user.foo /mnt/file;',
            )

            // Overwriting of xattr using xattrcreate.
            emulator.serial0_send(
                'setfattr --name=user.foo --value=baz /mnt/file;',
            )
            // File contents should not be overriden.
            emulator.serial0_send('cat /mnt/file;')
            emulator.serial0_send(
    ...
    },
    {
        ...
        start: () => {
            ...
            emulator.serial0_send('echo start-capture;')
            emulator.serial0_send(
                'getfattr --encoding=hex --absolute-names --name=security.capability /mnt/file;',
            )
    ...
    },
    {
        ...
        start: () => {
            ...
            emulator.serial0_send(
                "flock -s /mnt/file -c 'echo lock-shared-2 >> /mnt/file' \n",
            )
            emulator.serial0_send(
                "flock -xn /mnt/file -c 'echo lock unblocked! >> /mnt/logs' \n",
            )
            emulator.serial0_send('echo lock-shared-1 > /mnt/fifo1\n')

            emulator.serial0_send(
                "flock -x /mnt/file -c 'cat /mnt/fifo1 >> /mnt/file' &\n",
            )
            emulator.serial0_send(
                "flock -x /mnt/file -c 'echo lock-exclusive-2 >> /mnt/file' &\n",
            )
            emulator.serial0_send(
                "flock -sn /mnt/file -c 'echo lock unblocked! >> /mnt/logs' \n",
            )
            ...
            emulator.serial0_send(
                "flock -x /mnt/file -c 'echo lock-exclusive-3 >> /mnt/file' &\n",
            )
        ...
        end_trigger: 'done-locks',
            ...
            function test_getlock(
                ...
                const lock = emulator.fs9p.DescribeLock(
                    type,
                    pos,
                    1,
                    proc_id,
            ...
            function test_lock(
                ...
                console.log(
                    `    Lock ${num}: type=${type}, start=${start}, length=${length} ` +
    ...
    },
    {
        ...
        start: () => {
            ...
            emulator.serial0_send(
                '    echo "$f" > "/mnt/stress-files/file-$f"\n',
            )
            ...
            emulator.serial0_send(
                'test -z "$(ls /mnt/stress-files)" && echo delete-success\n',
            )
    ...
    },
    {
        ...
        start: () => {
            ...
            emulator.serial0_send(
                'test -z "$(ls /mnt/stress-dirs)" && echo delete-success\n',
            )
    ...
    },
    {
        ...
        start: () => {
            ...
            emulator.serial0_send(
                'dd if=/mnt/small-file bs=1 count=1 skip=100;',
            )
            emulator.serial0_send(
                'dd if=/mnt/small-file bs=1 count=1 skip=1000;',
            )
...
let test_num = 0
let test_timeout: ReturnType<typeof setTimeout>
let test_has_failed = false
const failed_tests: number[] = []
...
let capturing = false
let capture = ''
let next_trigger: string | undefined
let next_trigger_handler: (() => void) | undefined

async function prepare_test(): Promise<void> {
    console.log('\nPreparing test #%d: %s', test_num, tests[test_num].name)

    if (tests[test_num].timeout) {
        test_timeout = setTimeout(() => {
            console.error(
                '[-] Test #%d (%s) took longer than %s sec. Timing out and terminating.',
                test_num,
                tests[test_num].name,
                tests[test_num].timeout,
            )
            process.exit(1)
