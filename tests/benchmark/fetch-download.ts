...
const BENCHFILE_SIZE =
    (parseInt(process.env.BENCHFILE_SIZE_MB!, 10) || 32) * 1024 * 1024
...
if (isMainThread) {
    ...
    emulator.add_listener('serial0-output-byte', function (byte: number) {
        ...
        if (!booted && serial_text.endsWith('~% ')) {
            ...
            emulator.serial0_send(
                `udhcpc;curl --fail --connect-timeout 10 -s -o /dev/null -w '<%{exitcode}><%{speed_download}>\\t<DONE>' http://${SERVER_PORT}.external\n`,
            )
        }
    ...
    const benchsize: number = workerData
...
function parse_console(output: string): void {
    ...
    if (exitcode !== 0) {
        console.error(
            'Bench failed, curl returned non-zero exit code %s',
            exitcode,
        )
