...
async function run_test(name: string, config: any): Promise<void> {
    ...
    const lines: string[] = emulator.screen_adapter.get_text_screen()
    if (!lines.some((line: string) => line.startsWith('testpassed'))) {
        console.warn('Failed: ' + name)
        console.warn(
            lines.map((line: string) =>
                line.replaceAll(String.fromCharCode(0), ' '),
            ),
        )
        process.exit(1)
    }
