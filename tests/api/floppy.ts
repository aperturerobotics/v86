...
function regexp_escape(text: string): string {
...
async function exec_test(
    test_name: string,
    v86_config: any,
    timeout_sec: number,
    test_function: (emulator: any) => Promise<void>,
): Promise<void> {
    ...
    await new Promise<void>((resolve) =>
        emulator.bus.register('emulator-started', () => resolve()),
    )
    try {
    ...
    console.log(
        'Done: ' +
            test_name +
            ' (' +
            ((performance.now() - tm_start) / 1000).toFixed(2) +
            ' sec)',
    )
}
...
async function expect(
    ...
    if (command) {
        ...
        expected = [
            new RegExp(regexp_escape(command.trimRight()) + '$'),
            ...expected,
        ]
        await pause(100)
    }
    if (
        !(await emulator.wait_until_vga_screen_contains(expected, {
...
await exec_test(
    ...
    async (emulator: any) => {
        ...
        await emulator.set_fda({
            url: __dirname + '/../../images/freedos722.img',
        })
...
await exec_test(
    ...
    async (emulator: any) => {
        ...
        await expect(
            emulator,
            '\n',
            [/Volume Serial Number is [0-9A-F-]+/, '', 'Format another (Y/N)?'],
            3000,
        )
...
await exec_test(
    ...
    async (emulator: any) => {
        ...
        await expect(
            emulator,
            '',
            [/BIOS default device boot in \d+ seconds\.\.\./],
            10000,
        )
        ...
        await emulator.set_fda({
            url: __dirname + '/../../images/windows101.img',
        })
        ...
        await expect(
            emulator,
            'ls /mnt/fd0/COMMAND.COM\n',
            ['/mnt/fd0/COMMAND.COM', 'tc@box:~$'],
            3000,
        )
        ...
        await expect(
            emulator,
            'sudo mke2fs -q /dev/fd0\n',
            [
                "/dev/fd0 contains a vfat file system labelled 'WIN101'",
                'Proceed anyway? (y,N)',
            ],
            3000,
        )
