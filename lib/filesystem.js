...
FS.prototype.CreateTextFile = async function (filename, parentid, str) {
    ...
    if (this.is_forwarder(parent_inode)) {
        const foreign_parentid = parent_inode.foreign_id
        const foreign_id = await this.follow_fs(parent_inode).CreateTextFile(
            filename,
            foreign_parentid,
            str,
        )
...
FS.prototype.CreateBinaryFile = async function (filename, parentid, buffer) {
    ...
    if (this.is_forwarder(parent_inode)) {
        const foreign_parentid = parent_inode.foreign_id
        const foreign_id = await this.follow_fs(parent_inode).CreateBinaryFile(
            filename,
            foreign_parentid,
            buffer,
        )
...
FS.prototype.SearchPath = function (path) {
    ...
    for (var i = 0; i < n; i++) {
        ...
        if (id === -1) {
            if (i < n - 1)
                return { id: -1, parentid: -1, name: walk[i], forward_path } // one name of the path cannot be found
    ...
    for (var i = 1; i < this.inodes.length; i++) {
        ...
        if (this.IsDirectory(i)) {
            ...
            for (const [name, id] of inode.direntries) {
                if (name.length === 0) {
                    dbg_log(
                        'Error in filesystem: inode with no name and id ' + id,
                        LOG_9P,
                    )
                }
...
FSLockRegion.prototype.conflicts_with = function (region) {
    if (this.proc_id === region.proc_id && this.client_id === region.client_id)
        return false
    if (this.type === P9_LOCK_TYPE_UNLCK || region.type === P9_LOCK_TYPE_UNLCK)
        return false
    if (this.type !== P9_LOCK_TYPE_WRLCK && region.type !== P9_LOCK_TYPE_WRLCK)
        return false
